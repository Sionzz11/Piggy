 ...
 */
async function loadExchangeIds() {
    if (exchangeCacheLoaded)
        return;
    try {
        ...
        for (const ex of exchanges) {
            ...
            const symOf = (addr) => {
                ...
                for (const sym of ["USDm", "USDT", "USDC", "wETH"]) {
                    if (getTokenAddress(CHAIN_ID, sym).toLowerCase() === lower)
                        return sym;
                }
                return undefined;
            };
            ...
            exchangeIdCache.set(`${sym0}/${sym1}`, ex.exchangeId);
            exchangeIdCache.set(`${sym1}/${sym0}`, ex.exchangeId);
        }
 ...
 */
async function getAmountOut(from, to, amountIn) {
