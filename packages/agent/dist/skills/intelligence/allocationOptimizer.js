...
// ── Constants ──────────────────────────────────────────────────────────────
const STATIC_DEFAULT = { usdm: 1000, usdc: 3000, usdt: 6000 };
 ...
 */
export function optimizeAllocation(apys, userConstraints) {
    ...
    const allocation = { usdm: finalUsdm, usdc: finalUsdc, usdt: finalUsdt };
    const blendedApy = apys.usdm * (finalUsdm / 10000) +
        apys.usdc * (finalUsdc / 10000) +
        apys.usdt * (finalUsdt / 10000);
    const driftFromDefault = {
        usdm: finalUsdm - STATIC_DEFAULT.usdm,
        usdc: finalUsdc - STATIC_DEFAULT.usdc,
        usdt: finalUsdt - STATIC_DEFAULT.usdt,
    };
    const reason = `Optimized: USDm ${(finalUsdm / 100).toFixed(0)}% / ` +
