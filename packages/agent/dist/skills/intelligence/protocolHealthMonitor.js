...
async function checkMento() {
    ...
    try {
        ...
        const amountOut = await publicClient.readContract({
            ...
            args: [usdmAddr, usdcAddr, 1000000n],
        });
        if (amountOut === 0n) {
            status = "degraded";
    ...
    const report = { protocol: "mento", status, details, warnings };
...
async function checkUniswap() {
    ...
    const warnings = [];
    let status = "healthy";
    const POOL_ADDR = process.env.UNISWAP_USDC_WETH_POOL;
    if (!POOL_ADDR) {
    ...
    try {
        const [slot0, liquidity] = await Promise.all([
            publicClient.readContract({ address: POOL_ADDR, abi: UNISWAP_POOL_ABI, functionName: "slot0" }),
            publicClient.readContract({ address: POOL_ADDR, abi: UNISWAP_POOL_ABI, functionName: "liquidity" }),
        ]);
        if (!slot0.unlocked) {
            status = "unavailable";
    ...
    const report = { protocol: "uniswap", status, details, warnings };
...
export async function checkProtocolHealth() {
    ...
    const overallStatus = hasUnavailable ? "unavailable" :
