 ...
 */
const STALE_ESCALATION_COUNT = parseInt(process.env.PEG_STALE_ESCALATION_COUNT ?? "3");
...
const staleCounters = new Map();
function getStaleCount(token) {
    return staleCounters.get(token) ?? 0;
}
function incrementStale(token) {
    const next = getStaleCount(token) + 1;
    staleCounters.set(token, next);
    return next;
}
function clearStale(token) {
 ...
 */
async function fetchPriceVsMento(token, mentoAddress) {
    try {
...
function classifyDeviation(deviationPct) {
    if (deviationPct >= CRITICAL_PCT)
        return "critical";
    if (deviationPct >= ALERT_PCT)
        return "alert";
    if (deviationPct >= WARN_PCT)
        return "warn";
    return "ok";
}
function buildMessage(token, priceUSD, deviationPct, status) {
...
const PEG_STATUS_ORDER = ["ok", "warn", "alert", "critical"];
...
export async function checkStablecoinPegs() {
    ...
    for (const token of stables) {
        ...
        if (price !== null) {
            ...
            const status = escalated ? "alert" : "warn";
    ...
    const worstStatus = readings.reduce((worst, r) => worstOf(worst, r.status), "ok");
    return {
        readings,
        worstStatus,
...
export function _resetStaleCountersForTesting() {
    staleCounters.clear();
}
