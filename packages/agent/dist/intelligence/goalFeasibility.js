 ...
 */
function fvLumpSum(principal, annualRate, months) {
    if (annualRate === 0)
        return principal;
 ...
 */
function fvAnnuity(monthlyPayment, annualRate, months) {
    if (monthlyPayment <= 0)
        return 0;
    if (annualRate === 0)
 ...
 */
function requiredPMT(targetFV, annualRate, months) {
    if (targetFV <= 0)
        return 0;
    if (annualRate === 0)
 ...
 */
export function analyzeGoalFeasibility(input) {
    const { currentBalance, goalAmount, timeHorizonMonths, expectedAPY, plannedMonthlyDeposit = 0, } = input;
