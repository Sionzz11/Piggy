...
export async function allocateSavings(input) {
    try {
        ...
        const swaps = []; // tidak dipakai lagi — swap+supply sudah atomic
        const supplies = [
            // 60%: USDm → USDT → Aave (atomic)
            {
                ...
                data: encodeFunctionData({
                    ...
                    args: [
                        ...
                        (usdtAmount6 * SLIPPAGE99_BPS) / 10000n, // minAmountOut: USDT (6-dec)
                        (usdtAmount6 * SLIPPAGE99_BPS) / 10000n, // minATokens: aUSDT (6-dec)
                    ],
                }),
                value: 0n,
            ...
            // 30%: USDm → USDC → Aave (atomic)
            {
                ...
                data: encodeFunctionData({
                    ...
                    args: [
                        ...
                        usdcAmount18,
                        (usdcAmount6 * SLIPPAGE99_BPS) / 10000n,
                        (usdcAmount6 * SLIPPAGE99_BPS) / 10000n,
                    ],
                }),
                value: 0n,
