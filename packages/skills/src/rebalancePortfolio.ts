...
export interface CurrentAllocations {
  usdt: bigint;  // current aUSDT balance
  usdc: bigint;  // current aUSDC balance
  usdm: bigint;  // current aUSDm balance
}
...
export interface RebalanceOutput {
  ...
  skipReason?:     string;
  newAllocBps:     { usdt: number; usdc: number; usdm: number };
 ...
 */
function computeOptimalAlloc(
  apys: CurrentApys,
  currentBps: { usdt: number; usdc: number; usdm: number }
): { usdt: number; usdc: number; usdm: number } {
...
): Promise<SkillResult<RebalanceOutput>> {
  try {
    const {
      userWallet, executorAddress, currentApys,
      currentAllocations, totalPortfolioValue,
      lastRebalancedAt, estimatedGasCostUSD,
    } = input;
    ...
    const blended =
      currentApys.usdt * (usdt18 / aaveSum) +
    ...
    const newBlended =
      (currentApys.usdt * newAlloc.usdt +
...
function ok(data: RebalanceOutput): SkillResult<RebalanceOutput> {
