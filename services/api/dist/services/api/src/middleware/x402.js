...
import { markPaymentUsed } from "@piggy/db";
/**
...
const USDC_ADDRESS = getTokenAddress(CHAIN_ID, "USDC");
...
function parsePaymentHeader(header) {
    if (!PAYMENT_HEADER_REGEX.test(header))
        return null;
    const colon = header.indexOf(":");
    return {
        txHash: header.slice(0, colon),
        payer: header.slice(colon + 1),
    };
}
/**
 ...
 */
export async function x402PaymentGate(req, reply) {
