...
if (!PRIVY_APP_ID || !PRIVY_APP_SECRET) {
    throw new Error("[auth] PRIVY_APP_ID and PRIVY_APP_SECRET must be set in .env\n" +
...
export function assertOwns(req, goalOwnerWallet) {
    const caller = req.verifiedWallet;
    if (!caller)
        return null;
    if (caller !== goalOwnerWallet.toLowerCase())
        return null;
    return caller;
}
