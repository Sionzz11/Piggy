import { upsertUser, upsertAgentWallet, createGoal, getGoalById, getActiveGoalByOwner, updateGoalStatus, getAllActiveGoals, getRecentHistory, insertExecution, updateExecution, setSoftPausedById, clearGoalActionRequired, getLatestAgentEvent, getRecentAgentEvents, } from "@piggy/db";
...
import { requireAuth, assertOwns } from "../middleware/auth.js";
...
export async function goalsRoutes(app) {
    ...
    // GET /api/goals/all
    app.get("/all", { preHandler: requireAuth }, async (req, reply) => {
        const { verifiedWallet } = req;
    ...
    app.post("/create", { preHandler: requireAuth }, async (req, reply) => {
        const { verifiedWallet } = req;
        const { agentWalletAddress, targetAmount, targetCurrency, deadlineDate, spendLimit, } = req.body;
    ...
    app.post("/:id/activate", { preHandler: requireAuth }, async (req, reply) => {
        const goal = await getGoalById(req.params.id);
        if (!goal)
            return reply.code(404).send({ error: "goal not found" });
        if (!assertOwns(req, goal.ownerWallet)) {
        ...
        try {
            ...
            const pos = await client.readContract({
                ...
                functionName: "positions", args: [goal.ownerWallet],
            });
            if (pos[0] === 0n) {
            ...
            logger.warn("activate: on-chain position check failed — proceeding", err);
        }
    ...
    app.post("/:id/withdraw", { preHandler: requireAuth }, async (req, reply) => {
        const goal = await getGoalById(req.params.id);
        if (!goal)
            return reply.code(404).send({ error: "goal not found" });
        if (!assertOwns(req, goal.ownerWallet)) {
    ...
    app.post("/:id/reactivate", { preHandler: requireAuth }, async (req, reply) => {
        const goal = await getGoalById(req.params.id);
        if (!goal)
            return reply.code(404).send({ error: "goal not found" });
        if (!assertOwns(req, goal.ownerWallet)) {
    ...
    app.get("/:id/agent-status", { preHandler: requireAuth }, async (req, reply) => {
        const goal = await getGoalById(req.params.id);
        if (!goal)
            return reply.code(404).send({ error: "goal not found" });
        if (!assertOwns(req, goal.ownerWallet)) {
    ...
    app.post("/:id/complete-action", { preHandler: requireAuth }, async (req, reply) => {
        const goal = await getGoalById(req.params.id);
        if (!goal)
            return reply.code(404).send({ error: "goal not found" });
        if (!assertOwns(req, goal.ownerWallet)) {
    ...
    app.get("/all-active", { preHandler: requireInternalSecret }, async () => getAllActiveGoals());
}
