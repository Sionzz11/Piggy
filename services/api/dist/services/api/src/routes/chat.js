...
async function getChatCount(wallet) {
...
async function incrementChatCount(wallet) {
    const month = getCurrentMonth();
    try {
        await db
            .insert(chatCounts)
        ...
        logger.warn("incrementChatCount failed", err);
    }
}
function buildUsageFooter(countBefore) {
...
function buildSystemPrompt(goalContext) {
    return [
...
async function buildGoalContext(wallet) {
    try {
        ...
        const projLine = projectedDaysToGoal === 0
            ...
            : projectedDaysToGoal !== null
                ? "Projected completion: ~" + projectedDaysToGoal + " days from now (" + (projectedDaysToGoal <= daysLeft
        ...
        logger.warn("buildGoalContext failed", err);
        return "Goal context temporarily unavailable. Answer generally about Piggy Sentinel.";
    }
}
async function callClaude(userMessage, goalContext, history) {
