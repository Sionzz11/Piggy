import { createTelegramLink, confirmTelegramLink, getTelegramChatId, getWalletForChatId, } from "@piggy/db";
...
export async function telegramRoutes(app) {
    ...
    app.post("/confirm-link", async (req, reply) => {
        const { code, chatId } = req.body;
        if (!code || !chatId)
            return reply.code(400).send({ error: "code and chatId required" });
    ...
    app.get("/link-status", async (req, reply) => {
