...
import { BLENDED_APY_PCT, APY_CHANGE_THRESHOLD_PCT, MIN_REBALANCE_AMOUNT, MAX_REBALANCE_INTERVAL_MS, MAX_GAS_TO_YIELD_RATIO_PCT, ALLOC_USDT_BPS, ALLOC_USDC_BPS, ALLOC_USDM_BPS, } from "@piggy/shared";
/**
...
export function makeDecision(input) {
    ...
    if (DEMO_MODE && !softPaused && input.goalStatus !== "paused") {
        ...
        return execute("execute_rebalance", portfolio.lpUSD > 0 ? "lp" : "stable", newBlended, `[DEMO MODE] bypassing guardrails — blended APY ${newBlended.toFixed(2)}%`);
    }
    // ── Guardrail 1: paused ────────────────────────────────────────────────────
    ...
    if (portfolio.totalUSD < MIN_REBALANCE_AMOUNT) {
        return skip("skip_min_amount", `portfolio $${portfolio.totalUSD.toFixed(2)} < min $${MIN_REBALANCE_AMOUNT}`, BLENDED_APY_PCT);
    }
    ...
    if (apyDrift < APY_CHANGE_THRESHOLD_PCT) {
        return skip("skip_no_change", `APY drift ${apyDrift.toFixed(2)}% < threshold ${APY_CHANGE_THRESHOLD_PCT}%`, newBlended);
    }
    ...
    if (gasToYieldRatioPct > MAX_GAS_TO_YIELD_RATIO_PCT) {
        return skip("skip_gas_cost", `gas/yield ratio ${gasToYieldRatioPct.toFixed(1)}% > max ${MAX_GAS_TO_YIELD_RATIO_PCT}%`, newBlended);
    }
    ...
    if (input.riskScore && (input.riskScore.level === "high" || input.riskScore.level === "critical")) {
        return skip("skip_high_risk", `Risk score ${input.riskScore.score}/100 (${input.riskScore.level}) — dominant factor: ${input.riskScore.dominantFactor}. Holding steady.`, newBlended);
    }
    ...
    if (input.protocolHealth?.hasDegraded) {
        ...
        return skip("skip_protocol_degraded", `Protocol(s) degraded: ${degradedProtocols}. Skipping rebalance until health is restored.`, newBlended);
    }
    // ── All guardrails passed — execute rebalance ─────────────────────────────
    const tier = portfolio.lpUSD > 0 ? "lp" : "stable";
    return execute("execute_rebalance", tier, newBlended, `APY drift ${apyDrift.toFixed(2)}% — rebalancing from ${currentBlended.toFixed(2)}% → ${newBlended.toFixed(2)}%`);
}
// ── Helpers ───────────────────────────────────────────────────────────────────
function computeBlended(apys) {
    return (apys.usdt * (ALLOC_USDT_BPS / 10_000) +
        apys.usdc * (ALLOC_USDC_BPS / 10_000) +
        apys.usdm * (ALLOC_USDM_BPS / 10_000));
}
function skip(action, reason, estApy) {
    return { action, tier: "stable", reason, estimatedNewApy: estApy, shouldNotify: false };
}
function execute(action, tier, estApy, reason) {
