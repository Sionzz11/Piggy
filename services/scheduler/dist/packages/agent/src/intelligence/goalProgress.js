 ...
 */
function monthsToReachTarget(currentBalance, monthlyDeposit, annualRate, target) {
    if (currentBalance >= target)
        return 0;
    if (annualRate === 0 && monthlyDeposit <= 0)
        return null;
 ...
 */
export function computeGoalProgress(input, previousProgressPercent = 0) {
    const { currentBalance, goalAmount, startingBalance, goalStartDate, goalDeadline, expectedAPY, monthlyDeposit, } = input;
    ...
    const newMilestone = MILESTONES.find(m => progressPercent >= m && previousProgressPercent < m) ?? null;
