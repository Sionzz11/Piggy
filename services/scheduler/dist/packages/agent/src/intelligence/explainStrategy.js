...
function allocLabel(alloc) {
    const parts = [];
    if (alloc.stableBps > 0)
        parts.push(`${alloc.stableBps / 100}% in Aave stable yield`);
    if (alloc.lpBps > 0)
        parts.push(`${alloc.lpBps / 100}% in Uniswap liquidity`);
    if (alloc.wethBps > 0)
        parts.push(`${alloc.wethBps / 100}% in WETH`);
    return parts.join(", ");
}
/**
 * Generate an explanation for the agent's rebalance decision.
 */
export function explainRebalance(ctx) {
    ...
    if (action === "execute_initial_alloc") {
        ...
        const detail = `Your portfolio (${tierLabel(tier)} tier) is now allocated as: ${allocLabel(targetAlloc ?? { stableBps: 0, lpBps: 0, wethBps: 0 })}. ` +
 ...
 */
export function explainILExit(positionCount, ilPercent) {
 ...
 */
export function explainAllocation(alloc, estimatedApy, tier) {
    const lines = [
