...
// ─────────────────────────────────────────────────────────────────────────────
import { pgTable, uuid, text, numeric, boolean, timestamp, jsonb, index, integer, primaryKey, } from "drizzle-orm/pg-core";
// ── users ─────────────────────────────────────────────────────────────────────
...
export const goals = pgTable("goals", {
    ...
    //   Durasi saving cycle user dalam detik.
    //   7 * 86400  = 604800  → mingguan
    //   30 * 86400 = 2592000 → bulanan
    //   Scheduler baca ini untuk resetSpendEpoch() sesuai pilihan user.
    //   Sebelum: tidak ada kolom ini → scheduler hardcode 30 hari → user mingguan
    //            tidak di-reset selama 3 minggu → SpendLimitExceeded padahal top-up.
    epochDuration: integer("epoch_duration").notNull().default(7 * 86400),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
